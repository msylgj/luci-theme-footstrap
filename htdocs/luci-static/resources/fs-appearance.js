...
function render() {
	...
	return uci.load('footstrap').then(build);
}
...
function build() {
	...
	const wallpaper = (() => {
		prefs.configureLoginBackground(
		...
		loginModeSelect.value = prefs.currentLoginWallpaper();
		const collectionInput = E('input', {
			'class': 'cbi-input-text', 'type': 'text', 'inputmode': 'numeric', 'pattern': '[0-9]*',
			'value': prefs.currentOnlineCollectionId()
		});
		const apiKeyInput = E('input', {
			'class': 'cbi-input-text', 'type': 'password', 'autocomplete': 'off', 'value': prefs.currentOnlineApiKey()
		});
		const extraParamsInput = E('input', {
			'class': 'cbi-input-text', 'type': 'text', 'value': prefs.currentOnlineExtraParams()
		});
		...
		sourceSelect.value = prefs.currentOnlineWallpaper();
		...
		resolutionSelect.value = prefs.currentOnlineResolution();
		...
		loginModeSelect.addEventListener('change', () => {
			prefs.applyLoginWallpaper(loginModeSelect.value);
			updateOnlineRows();
			refreshSave();
		});
		sourceSelect.addEventListener('change', () => {
			prefs.applyOnlineWallpaper(sourceSelect.value);
			updateOnlineRows();
			refreshSave();
		});
		collectionInput.addEventListener('input', () => {
			...
			prefs.applyOnlineCollectionId(collectionInput.value);
			refreshSave();
		});
		apiKeyInput.addEventListener('input', () => {
			prefs.applyOnlineApiKey(apiKeyInput.value.trim());
			refreshSave();
		});
		extraParamsInput.addEventListener('input', () => {
			prefs.applyOnlineExtraParams(extraParamsInput.value.trim());
			refreshSave();
		});
		resolutionSelect.addEventListener('change', () => {
			prefs.applyOnlineResolution(resolutionSelect.value);
			refreshSave();
		});
		updateOnlineRows();
